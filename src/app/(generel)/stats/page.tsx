  ...
  useEffect(() => {
    const fetchGitHubStats = async () => {
      try {
        ...
        const userResponse = await fetch("https://api.github.com/users/Khalid-Sifullah-Siam");
        if (!userResponse.ok) {
        ...
        const reposResponse = await fetch(
          "https://api.github.com/users/Khalid-Sifullah-Siam/repos?per_page=100&sort=updated"
        );
        if (!reposResponse.ok) {
